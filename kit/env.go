...
// GetEnvFromFile loads environment variables from the specified .env file.
...
func GetEnvFromFile(path string) error {
	if err := godotenv.Overload(path); err != nil {
		return fmt.Errorf("%w %q: %w", ErrEnvFileLoad, path, err)
	}
...
// GetEnvWithDefault retrieves an environment variable value with a default fallback.
