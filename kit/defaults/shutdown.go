...
// SetDefaultsForShutdown registers signal handlers, exit with timeout, and garbage collection for cache storage.
func SetDefaultsForShutdown(parent context.Context) context.Context {
	...
	shutdownHandlers = append(shutdownHandlers, func() {
		_, _ = store.GetStoreCache().GarbageCollect()
	})
...
// RunShutdownHandlers executes handlers in reverse order, exactly once.
