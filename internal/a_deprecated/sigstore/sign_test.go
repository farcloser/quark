...
func TestSign_NoSigningMethodConfigured(t *testing.T) {
	...
	opts := &sigstore2.SignOptions{
		ImageRef: *imageRef,
		Digest:   "sha256:abc123",
		Log:      discardLogger(),
	}

	err = sigstore2.Sign(ctx, opts)
	if err == nil {
		t.Fatal("expected error when no signing method configured")
	}

	if !errors.Is(err, sigstore2.ErrNoSigningMethod) {
...
-----END PRIVATE KEY-----`),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !errors.Is(err, sigstore2.ErrSigningFailed) {
				t.Errorf("expected ErrSigningFailed, got: %v", err)
			}

			if !errors.Is(err, sigstore2.ErrInvalidPrivateKey) {
...
func TestSign_ValidECPrivateKey_FailsOnPush(t *testing.T) {
	...
	regClient := registry.NewClient(&types.RegistryCredentials{Domain: "ghcr.io"}, discardLogger())
