...
import (
	...
	registry2 "github.com/farcloser/quark/internal/a_deprecated/registry"
...
func TestClient_CopyImage_InvalidSourceReference(t *testing.T) {
	...
	client := registry2.NewClient(&types.RegistryCredentials{Domain: "docker.io"}, discardLogger())
	dstClient := registry2.NewClient(&types.RegistryCredentials{Domain: "ghcr.io"}, discardLogger())
...
func TestClient_GetImage_NonExistent(t *testing.T) {
	...
	if !errors.Is(err, registry2.ErrGetImage) {
		t.Errorf("GetImage() error = %v, want error wrapping %v", err, registry2.ErrGetImage)
	}
}
