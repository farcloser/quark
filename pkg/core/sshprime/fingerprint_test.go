...
func TestGetFingerprinter_Singleton(t *testing.T) {
	...
	fp1 := sshprime.GetFingerprinter()
	fp2 := sshprime.GetFingerprinter()
...
func TestFingerprinter_TrustThenVerify_Success(t *testing.T) {
	...
	endpoint := &sshprime.Endpoint{Host: "test-trust-success.example.com", Port: 22}
	hostname := "test-trust-success.example.com:22"

	fp := sshprime.GetFingerprinter()
	fp.Trust(endpoint, fingerprint)

	defer fp.Clear(endpoint)
...
func TestFingerprinter_TrustThenVerify_Mismatch(t *testing.T) {
	...
	endpoint := &sshprime.Endpoint{Host: "test-trust-mismatch.example.com", Port: 22}
	...
	if !errors.Is(err, sshprime.ErrFingerprintMismatch) {
...
func TestFingerprinter_Verifier_UnknownHost(t *testing.T) {
	...
	if !errors.Is(err, sshprime.ErrFingerprintUnknownHost) {
...
func TestFingerprinter_Clear(t *testing.T) {
	...
	endpoint := &sshprime.Endpoint{Host: "test-clear.example.com", Port: 22}
