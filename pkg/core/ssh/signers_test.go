...
func TestSignerFromKey_NilKey(t *testing.T) {
	...
	signer, err := coressh.SignerFromKey(nil)

	if signer != nil {
		t.Error("expected nil signer for nil key")
	}

	if !errors.Is(err, coressh.ErrSignerNilKey) {
...
func TestSignerFromKey_UnencryptedKey(t *testing.T) {
	...
	signer, err := coressh.SignerFromKey(key)
	if err != nil {
...
func TestSignerFromKey_EncryptedKeyWithoutPassphrase(t *testing.T) {
	...
	key := &coressh.Key{
		Bytes:      pemBytes,
		Passphrase: nil, // No passphrase provided for encrypted key
	}
	...
	if !errors.Is(err, coressh.ErrSignerParseKey) {
		t.Errorf("expected ErrSignerParseKey, got: %v", err)
	}
}
...
func TestSignerFromAgent_EmptyFingerprint_ReturnsFirstSigner(t *testing.T) {
	...
	signer, err := coressh.SignerFromAgent("")
	if err != nil {
...
func TestSignerFromAgent_MatchingFingerprint(t *testing.T) {
	...
	signer, err := coressh.SignerFromAgent(targetFingerprint)
	if err != nil {
		t.Fatalf("SignerFromAgent failed: %v", err)
	}
...
func TestSignerFromAgent_NonMatchingFingerprint(t *testing.T) {
	...
	signer, err := coressh.SignerFromAgent("SHA256:nonexistentfingerprint")
...
func TestSignerFromAgent_NoSignersWithFingerprint(t *testing.T) {
	...
	signer, err := coressh.SignerFromAgent("SHA256:somefingerprint")
