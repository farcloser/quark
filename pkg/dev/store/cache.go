...
func (c *Cache) Acquire(digest string) (io.ReadCloser, io.WriteCloser, error) {
	resourceDir := filepath.Join(c.rootDir, digest)
	...
	if tryLockErr == nil {
		return c.acquireNoActiveWriter(resourceDir, dataPath, tempPath, readLockPath, writeLockPath, dirLock, lockFile, writeLock)
	}

	// Case 2: Active writer exists (couldn't get writeLock)
	return c.acquireActiveWriter(dataPath, tempPath, writeLockPath, dirLock, lockFile)
}
...
func (c *Cache) acquireNoActiveWriter(
	...
	if file, err := os.Open(dataPath); err == nil { //nolint:gosec // Path is content-addressed from SHA256
	...
	//nolint:gosec // Path is content-addressed from SHA256
	...
	writer := &cacheWriter{
		file:      file,
		hasher:    sha256.New(),
		digest:    filepath.Base(dir), // digest is the directory name
		dir:       dir,
		dataPath:  dataPath,
		tempPath:  tempPath,
		lockFile:  lockFile,
...
func (c *Cache) acquireActiveWriter(
	dataPath, tempPath, writeLockPath string,
	dirLock, lockFile *os.File,
) (io.ReadCloser, io.WriteCloser, error) {
	// Case 2a: Try to open the temp file (writer is actively writing)
	if file, err := os.Open(tempPath); err == nil { //nolint:gosec // Path is content-addressed from SHA256
		_ = filesystem.Unlock(dirLock)
...
//nolint:wrapcheck // Internal function, callers add context
...
func (w *cacheWriter) Close() error {
	...
	// Step 3: Compare hash with digest
	...
	return fmt.Errorf("%w: expected %s, got %s", fault.ErrHashMismatch, w.digest, actual)
}
...
func (r *inProgressReader) Read(p []byte) (int, error) {
	for {
		...
		if _, err := os.Stat(r.dataPath); err == nil {
			// Success - data file exists, we've read everything
			return 0, io.EOF
		}
